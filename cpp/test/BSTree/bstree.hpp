...
class BSTree {
    public:
        ...
        // Returns a linked list containing all data in a given depth.
        GList<tipo> depthList(unsigned int depth);
...
template<typename tipo>
static GList<tipo> depthListAux(Node<tipo>* root, unsigned int depth) {

    GList<tipo> list;
    if (root == nullptr) {
       return list; // This is an empty list due to GList constructor
    }
    else if (depth == 0) 
        list.agregarFinal(root->data);
    
    else {

        GList<tipo> list1 = depthListAux(root->left, depth - 1);
        GList<tipo> list2 = depthListAux(root->right, depth - 1);
...
template<class tipo>
// Returns a linked list containing all data in a given depth.
GList<tipo> BSTree<tipo>::depthList(unsigned int depth) {

    GList<tipo> list = depthListAux(root, depth);
